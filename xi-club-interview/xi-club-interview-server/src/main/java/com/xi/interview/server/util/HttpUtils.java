package com.xi.interview.server.util;
 ...
 *
 * 1. HttpUtils.java：HTTP 请求工具类
 * 封装了基于 Apache HttpClient 的 HTTP 请求能力，为调用大模型 API 提供网络通信支持：
 * 核心能力：
 * 初始化带连接池的 HttpClient（支持 HTTP/HTTPS、连接复用、超时 / 重试配置）；
 * 提供httpGet（GET 请求）、httpPostJson/executePost（POST JSON 请求）方法；
 * 支持自定义请求头、Cookie、Basic Auth 等；
 * 自动管理连接池（过期 / 空闲连接清理、JVM 关闭时释放连接）。
 * 关键设计：
 * 连接池配置（最大连接数 1000、每路由 200），避免频繁创建连接；
 * 超时配置（连接 / 响应 / 请求超时均为 5 秒），防止请求阻塞；
 * 重试策略（失败重试 2 次），提升请求稳定性。
 */
@Slf4j
